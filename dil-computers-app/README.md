...
Every amount in this app is **Indian rupees**. Money is stored as a plain
number — there is no currency field, because there is only ever one
currency — and formatted for display in exactly two places:

- `formatPrice` (`client/src/format.js`) — `en-IN` / INR, so amounts read
  `₹1,23,456.78` with Indian lakh/crore grouping rather than the Western
  thousands grouping.
- `formatPriceAscii` — the same number as `Rs. 1,23,456.78`. PDFs use this
  one because jsPDF's built-in fonts are WinAnsi-encoded and have no ₹
  glyph (U+20B9); a ₹ there renders as a stray box or drops out entirely.

The catalogue prices in `server/data/product_catalogue.csv` are rupee
figures as supplied — nothing is converted at read time, and no exchange
rate exists anywhere in the codebase.
...
### Monthly sales registers

`server/db/seed.js`'s `importMonthlySales` loads every
`server/data/<month>_<year>_sales.csv`, oldest month first, on every
deploy. It is idempotent per voucher number, so it only ever inserts what
isn't there yet — and adding a new month is just dropping its CSV in and
redeploying.

**April 2026 is real**: `april_2026_sales.csv`, 128 invoices exported from
a Tally "Sales - GST Register" covering 1–30 April 2026 (₹15,74,156.50
across 87 customers).

**May 2026 onward is synthetic**, generated by
`server/data/generate_monthly_sales.py` so the app has a full
year-to-date to chart against. The script reuses April's real customer
list and, per customer, the voucher types they actually buy and the range
of amounts they spend — but the dates and amounts themselves are invented.
Its RNG is seeded, so regenerating produces byte-identical files and can
never fork into a second set of invoices. Treat these months as shape,
not truth; re-run the script (or delete the files) when real exports
arrive.

Worth knowing about these imports:
...
- **It bypasses the normal `POST /api/invoices` flow deliberately.** That
  ...
  inventing a fake product to decrement hundreds of times (corrupting real stock
...
- Two of the four voucher types — `AMC-ANNUAL MAINTENANCE` and
  ...
  invoices, at the business owner's explicit choice, purely so the
  revenue totals are complete — they're distinguishable by category
