...
export default function PurchaseOrders({ token, onLogout }) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/purchase-orders?${params.toString()}`, token)
      .then((data) => {
        if (cancelled) return
        setItems(data.items)
        setTotal(data.total)
  ...
  return (
    <div>
      <header style={styles.header}>
        ...
        <p style={styles.subtitle}>{total.toLocaleString()} purchase orders</p>
      </header>
        ...
        <table style={styles.table}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td style={styles.td} colSpan={5}>No purchase orders match your search.</td></tr>
            ) : (
              items.map((po) => (
                <Fragment key={po.id}>
                  <tr style={styles.row} onClick={() => setExpandedId(expandedId === po.id ? null : po.id)}>
                    <td style={styles.td}>{po.po_number}</td>
                    <td style={styles.td}>{formatDate(po.po_date)}</td>
                    <td style={styles.td}>{po.supplier_name || '—'}</td>
                    <td style={styles.td}>{po.created_by_username || '—'}</td>
                    <td style={{ ...styles.td, textAlign: 'right' }}>{formatPrice(po.grand_total)}</td>
                  </tr>
                  {expandedId === po.id && (
                    <tr>
                        ...
                        <table style={styles.innerTable}>
                          <thead>
                            <tr>
                              ...
                              <th style={{ ...styles.innerTh, textAlign: 'right' }}>Cost Price</th>
                          ...
                          <tbody>
                            {(po.items || []).map((item, idx) => (
                              <tr key={idx}>
                                ...
                                <td style={{ ...styles.innerTd, textAlign: 'right' }}>{formatPrice(item.costPrice)}</td>
...
const styles = {
  ...
  row: { cursor: 'pointer' },
  detailCell: { padding: '12px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0' },
  innerTable: {
    width: '100%',
