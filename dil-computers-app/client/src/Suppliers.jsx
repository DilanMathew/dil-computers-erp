...
function SupplierDetail({ token, onLogout, supplierId, canEdit, onSaved }) {
  ...
  function load() {
    setLoading(true)
    apiFetch(`/api/suppliers/${supplierId}`, token)
      .then((data) => {
        setDetail(data)
        setForm({
  ...
  useEffect(load, [token, supplierId])

  async function handleSave() {
    setSaving(true)
    setError('')
    try {
      await apiFetch(`/api/suppliers/${supplierId}`, token, {
        method: 'PATCH',
  ...
  if (!detail) return <div style={styles.detailCell}>{error || 'Could not load supplier.'}</div>
    ...
    <div style={styles.detailCell}>
      {editing ? (
        <div style={styles.editGrid}>
          ...
          <input style={styles.input} value={form.gstin} onChange={(e) => setForm({ ...form, gstin: e.target.value })} placeholder="GSTIN" />
        ...
        <>
          <div style={styles.detailGrid}>
            {detail.supplier.email && <span>Email: {detail.supplier.email}</span>}
            {detail.supplier.address && <span>Address: {detail.supplier.address}</span>}
            {detail.supplier.gstin && <span>GSTIN: {detail.supplier.gstin}</span>}
            {detail.supplier.notes && <span>Notes: {detail.supplier.notes}</span>}
            {canEdit && (
          ...
          <div>
            <h4 style={styles.historyTitle}>Purchase orders ({detail.purchaseOrders.length})</h4>
            {detail.purchaseOrders.length === 0 ? (
              <p style={styles.historyEmpty}>None yet.</p>
            ) : (
              detail.purchaseOrders.map((po) => (
                <div key={po.id} style={styles.historyRow}>
                  <span>{po.po_number} · {formatDate(po.po_date)}</span>
                  <span>{formatPrice(po.grand_total)}</span>
                </div>
              ))
            )}
          </div>
...
export default function Suppliers({ token, user, onLogout }) {
  ...
  const [newSupplier, setNewSupplier] = useState({ name: '', phone: '', email: '', address: '', gstin: '', notes: '' })
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/suppliers?${params.toString()}`, token)
      .then((data) => {
        if (cancelled) return
        setItems(data.items)
        setTotal(data.total)
  ...
  async function handleCreate(e) {
    ...
    setCreating(true)
    try {
      await apiFetch('/api/suppliers', token, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(newSupplier),
      })
      setNewSupplier({ name: '', phone: '', email: '', address: '', gstin: '', notes: '' })
      setShowForm(false)
      setRefreshKey((k) => k + 1)
    } catch (err) {
      if (err instanceof AuthError) onLogout()
      else setFormError(err.message || 'Could not create supplier.')
    } finally {
      setCreating(false)
    }
  }
  ...
  return (
    <div>
      <header style={styles.header}>
        ...
        <p style={styles.subtitle}>{total.toLocaleString()} suppliers</p>
      </header>
      ...
      {showForm && canEdit && (
        <form style={styles.card} onSubmit={handleCreate}>
          <div style={styles.formGrid}>
            <input style={styles.input} placeholder="Name" value={newSupplier.name} onChange={(e) => setNewSupplier({ ...newSupplier, name: e.target.value })} />
            <input style={styles.input} placeholder="Phone" value={newSupplier.phone} onChange={(e) => setNewSupplier({ ...newSupplier, phone: e.target.value })} />
            <input style={styles.input} placeholder="Email" value={newSupplier.email} onChange={(e) => setNewSupplier({ ...newSupplier, email: e.target.value })} />
            <input style={styles.input} placeholder="Address" value={newSupplier.address} onChange={(e) => setNewSupplier({ ...newSupplier, address: e.target.value })} />
            <input style={styles.input} placeholder="GSTIN" value={newSupplier.gstin} onChange={(e) => setNewSupplier({ ...newSupplier, gstin: e.target.value })} />
          </div>
          {formError && <div style={styles.error}>{formError}</div>}
          <button type="submit" style={{ ...styles.addButton, marginTop: 12 }} disabled={creating}>
            {creating ? 'Adding…' : 'Save supplier'}
          </button>
        </form>
      )}
        ...
        <table style={styles.table}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td style={styles.td} colSpan={4}>No suppliers match your search.</td></tr>
            ) : (
              items.map((s) => (
                <Fragment key={s.id}>
